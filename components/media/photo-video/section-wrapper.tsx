...
export default function SectionWrapper({ children, direction, ...props }: SectionWrapperProps) {
  return (
    <motion.section
      ...
      className={cn(
        'relative flex h-fit w-full flex-col gap-medium overflow-hidden rounded-small border border-black border-opacity-40 bg-black95 bg-opacity-60   pt-medium  backdrop-blur-sm    mobile-large:h-full laptop:w-1/2  '
      )}
      {...props}
    >
      {children}
